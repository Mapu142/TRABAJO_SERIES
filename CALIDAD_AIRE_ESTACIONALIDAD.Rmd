...
La prueba de Ljung-Box aplicada al rezago 365 arroja un p-valor nulo (p ≈ 0), lo que implica evidencia estadísticamente significativa de autocorrelación estacional en los residuales.
...
start_future <- as.Date("2020-12-07")
fechas_future <- seq(start_future, by = "day", length.out = 365)

df_pred <- data.frame(
  Fecha = fechas_future,
  Pronostico = as.numeric(forecast_PM25),
...
ggplot(df_pred, aes(x = Fecha)) +
  ...
  geom_ribbon(aes(ymin = Lower80, ymax = Upper80),
              fill = "#4A90E2" , alpha = 0.6) +
  geom_line(aes(y = Pronostico), color = "blue", linewidth = 1.1) +
  labs(
    title = "Pronóstico de PM2.5 en Delhi (dic 2020 – dic 2021)",
    subtitle = "Modelo SARIMA(1,0,2)(0,1,0)[365]",
    x = "Fecha",
    y = "PM2.5 (µg/m³)"
  ) +
  theme_minimal()
...
# --- 1. Datos reales (2019–2021) ---
df_real_zoom <- aire %>%
  filter(Date >= as.Date("2019-01-01")) %>%
  select(Date, PM2.5)

# --- 2. Datos predichos (df_pred ya existe) ---
df_pred_zoom <- df_pred

# --- 3. Gráfico combinado CORREGIDO ---
ggplot() +
  geom_line(data = df_real_zoom,
            aes(x = Date, y = PM2.5),
            color = "gray70", alpha = 0.5, linewidth = 0.4) +
  
  geom_ribbon(data = df_pred_zoom,
              aes(x = Fecha, ymin = Lower95, ymax = Upper95),
              fill = "gray80", alpha = 0.4) +
  
  geom_ribbon(data = df_pred_zoom,
              aes(x = Fecha, ymin = Lower80, ymax = Upper80),
              fill = "#4A90E2", alpha = 0.35) +
  
  geom_line(data = df_pred_zoom,
            aes(x = Fecha, y = Pronostico),
            color = "#003f8c", linewidth = 1.2) +
  
  labs(
    title = "PM2.5 observado y pronosticado (Zoom 2019–2021)",
    subtitle = "Datos reales (gris) y pronóstico SARIMA (azul)",
    x = "Fecha",
    y = "PM2.5 (µg/m³)"
  ) +
  theme_minimal()
...
En la serie pronosticada se conserva el patrón estacional característico de la contaminación atmosférica en Delhi. El modelo anticipa valores elevados durante los meses invernales (especialmente diciembre, enero y noviembre), con promedios diarios que oscilan entre 150 y 300 µg/m³, e incluso con picos más altos según los intervalos de confianza. Este comportamiento coincide con la época del año en la que se presentan condiciones meteorológicas adversas, mayor uso de combustibles y episodios de quema agrícola en estados cercanos.

Durante los meses comprendidos entre marzo y junio, el pronóstico muestra una disminución gradual de los niveles de PM2.5, con valores generalmente menores a 100 µg/m³. Posteriormente, en los meses de julio y agosto, el modelo proyecta los niveles más bajos del ciclo anual (entre 60 y 90 µg/m³), lo cual es coherente con la temporada de monzones, donde las lluvias intensas contribuyen a la remoción de partículas en suspensión.

A partir de septiembre, se observa un incremento progresivo en los valores pronosticados, el cual se intensifica de manera marcada en octubre y noviembre, reproduciendo el inicio de un nuevo ciclo de alta contaminación. Este comportamiento refleja la fuerte estacionalidad anual del fenómeno y confirma que el modelo captura de manera adecuada la dinámica histórica de la serie.

Los intervalos de pronóstico del 80% y 95% muestran una mayor amplitud en los meses con niveles más altos de contaminación, reflejando la elevada variabilidad y la presencia de episodios extremos típicos del invierno y del periodo post-monzónico. En contraste, durante la época de lluvias los intervalos son más estrechos, indicando mayor estabilidad y menor incertidumbre en los valores esperados.

En conjunto, el pronóstico obtenido es consistente con los patrones observados en años anteriores y ofrece una estimación realista del comportamiento futuro del PM2.5 en Delhi, manteniendo las características de estacionalidad, variabilidad y picos extremos que definen la calidad del aire en la ciudad.
