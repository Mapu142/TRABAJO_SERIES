...
`auto.arima($\triangle$log(PM2.5))`). Adicinalmente se prueba un modelo ARIMA(2,1,2) puesto que se puede comparar la serie sin diferenciar propuesta con la misma pero diferenciandola.
...
$$ =0.216 \nabla Z_{t-1}+0.134 \nabla Z_{t-2}+a_{t}-0.428a_{t-1}-0.381a_{t-2} $$

Para el modelo ARIMA(2,1,2) parece "reducirce" a un modelo ARIMA(1,1,2)
# Validación modelos
## ARMA(2,2)
```{r}
...
```
La serie logaritmica parece tener un comportamiento similar a un modelo ARMA(2,2), lo que nos puede indicar que el modelo se puede ajustar bien a los datos.
```{r}
par(mfrow=c(1,2))
...
arima212_ar <- arima2.1.2$coef[grep("^ar", names(arima2.1.2$coef))]
arima212_ma <- arima2.1.2$coef[grep("^ma", names(arima2.1.2$coef))]
...
acf(d.l.PM2.5, main = "ACF empírica de $\triangledown$log(PM2.5)")
...
pacf(d.l.PM2.5, lag.max = 20, main = "PACF empírica de $\triangledown$log(PM2.5)", ylim = range(c(pacf_empirica$acf, -0.25, 0.25)) )
...
Ambos modelos parecen ajustarse bien a los datos, ambos modelos carecen de normalidad, por lo que las pruebas de significancia de los parámetros no son válidas, así mismo pierden válidez los criterios AIC y BIC, sin embargo, teniendo en cuenta que ambos modelos se comportan de manera similar, y que el modelo ARMA(2,2) tiene menor RMSE, se ha seleccionado como el mejor modelo de ambos.
