...
# Extraer coeficientes y estadísticos
# ARMA(2,2): tiene intercepto, ar1, ar2, ma1, ma2
coef_arma <- coef(arma2.2)
t_arma <- coef_arma / sqrt(diag(arma2.2$var.coef))

# ARIMA(1,1,2): NO tiene intercepto, solo ar1, ma1, ma2 → ar2 = NA
coef_arima <- c(NA, arima2.1.2$coef["ar1"], NA, 
                arima2.1.2$coef["ma1"], arima2.1.2$coef["ma2"])
t_arima <- c(NA, 
             arima2.1.2$coef["ar1"] / sqrt(arima2.1.2$var.coef["ar1","ar1"]),
             NA,
             arima2.1.2$coef["ma1"] / sqrt(arima2.1.2$var.coef["ma1","ma1"]),
             arima2.1.2$coef["ma2"] / sqrt(arima2.1.2$var.coef["ma2","ma2"]))

# Construir tabla
resumen <- data.frame(
  Parámetro = parametros,
  
  "ARMA(2,2)" = c(coef_arma, arma2.2$loglik, arma2.2$aic, BIC(arma2.2)),
  "t-valor ARMA(2,2)" = c(t_arma, NA, NA, NA),
  
  "ARIMA(1,1,2)" = c(coef_arima, arima2.1.2$loglik, arima2.1.2$aic, BIC(arima2.1.2)),
  "t-valor ARIMA(1,1,2)" = c(t_arima, NA, NA, NA),
  stringsAsFactors = FALSE
)

knitr::kable(resumen,
             caption = "Tabla de resumen de los modelos ARMA(2,2) y ARIMA(1,1,2)",
             digits = 3,
             row.names = FALSE)
```
