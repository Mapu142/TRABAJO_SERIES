...
# Modelo estructural en espacio–estado

A partir de la serie diaria de PM2.5 (transformada logarítmicamente si se desea estabilizar la varianza), se plantea un modelo estructural en forma de espacio–estado compuesto por un nivel y un componente estacional anual.
...
\right]
$$ donde $q$ es el número de armónicos utilizados para aproximar el ciclo anual.

```{r fig.height=5, fig.width=10}

y3 <- l.PM2.5

# Modelo optimizado: Nivel local + Estacional trigonométrica (4 armónicos para anual)
...
residuos <- residuals(y3_suave, type = "raw")
...
# d) Histograma + QQ
hist(residuos$res, breaks = 30, main = "Distribución de residuos", xlab = "Residuo")
qqnorm(residuos$res); qqline(residuos$res, col = "red")
```
