...
\right]
$$ 
donde $q=8$ es el número de armónicos utilizados para aproximar el ciclo anual.
...
mod_trig <- dlmModPoly(order=1, dV=2, dW=0) +  # Nivel constante (W=0 para determinístico)
            dlmModTrig(s=365, q=8, dV=0, dW=0)  # s=periodo, q=armónicos, varianzas fijas
...
nivel_filtro <- dropFirst(y3_suave$s)[,1]
estacional_filtro <- rowSums(dropFirst(y3_suave$s)[,2:ncol(y3_suave$s)])  # Suma armónicos

# Plots comparativos (similar a tu código)
par(mfrow=c(1,1))
ts.plot(y3, col="darkgrey", main="Serie vs Nivel + Estacional (Suavizado)")
lines(nivel_filtro + estacional_filtro, col="red", lty=2)
```
La Figura muestra la serie observada de PM2.5 en escala logarítmica (línea gris) junto con el ajuste obtenido a partir del modelo estructural en espacio–estado (línea roja discontinua), correspondiente a la suma del nivel estimado y la componente estacional anual. Se observa que el modelo logra capturar adecuadamente la dinámica general de la serie, reproduciendo de forma clara el patrón estacional anual caracterizado por picos recurrentes y valles pronunciados. Asimismo, el nivel estimado presenta una evolución suave en el tiempo, lo que sugiere cambios graduales en la concentración promedio de PM2.5. En conjunto, el ajuste indica que el modelo estructural proporciona una representación adecuada de la serie.


## Validación del modelo: análisis de residuos del suavizado

Los residuales se definieron como la diferencia entre la serie observada y el valor ajustado suavizado del modelo.
...
ts.plot(y3, col = "darkgrey", main = "Serie vs Ajuste")
lines(ajuste, col = "red")
...
# d) Histograma + QQ
hist(residuos, breaks = 30, main = "Distribución de residuos", xlab = "Residuo")
qqnorm(residuos); qqline(residuos, col = "red")
```
En la primera imagen vemos el ajuste del modelo en rojo junto con la serie logarítmica en gris, se puede ver que el modelo reproduce adecuadamente el nivel y la estacionalidad principal de la serie, capturando la dinámica de largo plazo y el patrón anual dominante. El ajuste suavizado sigue de cerca la trayectoria promedio de los datos, aunque no pretende capturar la variabilidad de alta frecuencia.
...
```{r}
# Ljung-Box para varios rezagos
Box.test(residuos, lag = 20, type = "Ljung-Box")
```
Debido a que se obtuvo un p-valor menor al nivel de significancia (0.05) no se rechaza la posible autocorrelación estacional en los residuales, confirmando las sospechas que se tenían del ACF de estos.

```{r}
tseries::jarque.bera.test(residuos)
```
Con un valor tan pequeño en la prueba, rechazamos la hipótesis de que los residuales vienen de uns distribución normal

```{r}
bptest(residuos ~ time(residuos))
```
Con un valor pequeño rechazamos la hipótesis de igualdad de varianzas.
