...
```{r}
# Ajuste suavizado (numérico)
ajuste <- as.numeric(nivel_filtro + estacional_filtro)

# Residuos in-sample (numérico)
residuos <- as.numeric(y3) - ajuste

# (Opcional) volver a ts para plots bonitos
residuos_ts <- ts(residuos, start = start(y3), frequency = frequency(y3))
ajuste_ts   <- ts(ajuste,   start = start(y3), frequency = frequency(y3))

par(mfrow=c(3,2))
ts.plot(y3, col="darkgrey", main="Serie vs ajuste suavizado")
lines(ajuste_ts, col="red")

plot(residuos_ts, main="Residuos (in-sample)", ylab="Residuo")
Acf(residuos_ts, main="ACF residuos")
hist(residuos, breaks=30, main="Hist residuos", xlab="Residuo")
